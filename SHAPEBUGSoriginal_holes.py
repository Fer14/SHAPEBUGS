...
holex = 300 + int(Rect_x/4)
holey = 300 + int(Rect_y/4)
rholex = int(Rect_x/2)
rholey = int(Rect_y/2)
...
class SHAPEBUG:
    ...
    def draw(self,screen):
        ...
        if self.islocalized and flag == 'si':
            pygame.draw.circle(screen,ROSE,self.believed_location,tam_robots)
    ...
    def setlocation(self,x,y):
        self.x = x
        self.y = y
        if is_inside_figure((self.believed_location[0],self.believed_location[1])):
            self.set_state("moving_inside")
    ...
    def move_forward(self):
        ...
        if ((self.get_state() == "moving_inside") and (not is_inside_figure((self.get_believed_location()[0]+x,self.get_believed_location()[1]+y)))):
            self.set_state('end')
    ...
    def init_believed_location(self):
        if self.isseed == False:
            self.believed_location = (300,300)
    ...
    def update_believed_location(self):
        ...
        for neighbor in newrobot:
            ...
            if ((neighbor.get_location() != self.get_location()) and (neighbor.get_believed_location() != self.get_believed_location())):
        ...
        if (len(nlist) >= 3):
            for l in nlist:
                ...
                if (c == 0):
                    '''print('error')
                    pygame.draw.circle(screen,ROSE,self.get_location(),tam_robots)
                    pygame.draw.circle(screen,BLACK,self.get_believed_location(),tam_robots)
                    pygame.draw.circle(screen,GREEN,l.get_believed_location(),tam_robots)
                    pygame.draw.circle(screen,BLUE,l.get_location(),tam_robots)
                    pygame.display.update()
                    print(l.get_location())
                    print(self.get_location())
                    print(l.get_believed_location())
                    print(self.get_believed_location())
                    print((l.get_location() != self.get_location()) and (l.get_believed_location() != self.get_believed_location()))'''
        ...
        if not self.is_seed():
            ...
            if self.get_state() != 'end':
                ...
                if self.get_state() == 'start':
                    ...
                    self.edge_following()
                    #print(is_inside_figure(self.get_location()))
...
def is_inside_figure(location):
    x,y = location
    
    if  point_inside_rec(300,300, Rect_x, Rect_x,x,y) and not point_inside_rec(holex,holey,rholex,rholey,x,y):
        return True
...
pygame.draw.rect(screen,WHITE,(holex,holey,rholex,rholey))
